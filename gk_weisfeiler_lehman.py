...
class GK_WL():
	...
	def compare_list(self, graph_list, h = 1, node_label = True):
		...
		for i in range(n):
			t = []
			for a, b in graph_list[i].adjacency():
				t.append(list(b.keys()))
			lists[i] = copy.copy(t)
		...
		phi = np.zeros((n_max, n), dtype = np.int32)
		...
		if node_label is True:
			...
			for i in range(n):
				t = []
				for a, b in graph_list[i].degree():
					t.append(b)
				labels[i] = np.array(t)
		...
		while it < h:
			...
			phi = np.zeros((n_nodes, n), dtype = np.int32)
			for i in range(n):
				for v in range(len(lists[i])):
					...
					long_label = np.concatenate((np.array([labels[i][v]]),
					                             np.sort(
						                             labels[i][lists[i][v]])))
					long_label_string = ','.join(
						[str(x) for x in long_label.tolist()])
				...
				aux = np.bincount(new_labels[i]).astype(np.int32)
...
def compare(a, b, labela, labelb):
	...
	wl = GK_WL()
	r = wl.compare(g0, g1, node_label = True)
	return r
...
if __name__ == '__main__':
	...
	for i in range(0, 9):
		for j in range(i + 1, 9):
			g3.add_edge(i, j)
	wl = GK_WL()
	print(wl.compare(g0, g1, node_label = False))
	print(wl.compare(g2, g0, node_label = False))
	print(wl.compare(g3, g0, node_label = False))
